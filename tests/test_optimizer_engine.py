...
@pytest.fixture
def mock_ohlcv_data():
    # Create 14 days of 1-minute bar data (20000 rows)
    # This is enough to resample into 1h and 4h bars with periods of 5
    date_range = pd.date_range(end=pd.Timestamp.now(), periods=20000, freq='1min')
    
    # Generate some simple trends so strategy can trigger entry/exits
    np.random.seed(42)
    close_prices = 50000.0 + np.cumsum(np.random.randn(20000) * 200)
    open_prices = close_prices - np.random.randn(20000) * 5
    high_prices = np.maximum(open_prices, close_prices) + np.abs(np.random.randn(20000) * 10)
    low_prices = np.minimum(open_prices, close_prices) - np.abs(np.random.randn(20000) * 10)
    volume = np.random.randint(10, 1000, size=20000)
...
async def test_optimizer_engine_find_best_params(mock_config, mock_ohlcv_data):
    ...
    with patch('src.optimizer_engine.BinanceDataFetcher') as MockFetcher:
        ...
        with patch('pandas.read_csv', return_value=mock_ohlcv_data):
            # Run the optimizer
            best_params = await engine.find_best_params("BTC/USDT", lookback_days=12)
