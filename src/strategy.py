...
from src.strategy_numba import numba_check_entry, numba_check_exit, numba_find_first_exit, numba_check_entry_scalper, numba_check_exit_scalper, numba_find_first_exit_scalper
...
class TrendCrusherScalper(BaseStrategy):
    def __init__(self, config=CONFIG):
        self.c = config
        self.capital = config.get("SEED", 10000)
        self.position, self.entry_price, self.quantity = 0, 0, 0
    ...
    def check_entry_signal(self, row, last_price, use_sniper=False, retest_maker=False, config=None, is_ambushing=False):
        c = config if config else self.c
        sig_idx, target_p, sl_p = numba_check_entry_scalper(
            last_price, row['ema_h'], row['upper'], row['lower'], row['atr'], row['adx'], row['avg_vol'], row['volume'],
            c.get("VOL_MULTIPLIER", 2.0), c.get("ADX_FILTER_LEVEL", 25.0), c.get("INITIAL_SL_ATR", 2.0),
            use_sniper, retest_maker, c.get("SNIPER_PROXIMITY_PCT", 0.005), is_ambushing,
            c.get("INITIAL_SL_PCT", 0.0), row.get('adx_4h', 0.0), c.get("ADX_4H_THRESHOLD", 20.0),
    ...
    def check_entry(self, last_price, ema_h, upper, lower, atr, adx, avg_vol, volume, 
                    vol_mult, adx_threshold, initial_sl_atr, use_sniper, retest_maker, 
                    prox_threshold, is_ambushing, fixed_sl_pct, adx_4h, adx_4h_threshold,
                    chop, ema_slope, chaos, chaos_threshold, squeeze, config):
        return numba_check_entry_scalper(
    ...
    def check_exit_signal(self, row, last_price, state, config):
        ...
        be_guard = config.get("BE_GUARD_THRESHOLD_SCALPER", config.get("BE_GUARD_THRESHOLD", 0.0))
    ...
    def find_first_exit(self, closes, lookup_indices, position, entry_price, initial_max, initial_min, initial_sl,
                         atrs, atr_trail_mult, use_adaptive, adaptive_steps_arr, be_guard_threshold, config):
        be_guard = config.get("BE_GUARD_THRESHOLD_SCALPER", be_guard_threshold)
