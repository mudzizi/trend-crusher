...
def optimize_range(df_1m, start_date, end_date):
    # Filter 1m data for the specific range
    data_1m = df_1m[(df_1m['timestamp'] >= start_date) & (df_1m['timestamp'] < end_date)].copy()
    
    if len(data_1m) < 1440: # At least 1 day of 1m data
        return None
    ...
    for idx, (vol, trail, adx, don, risk, (m_name, sniper, retest), adapt) in enumerate(combinations):
        strategy = TrendCrusherV2(config=CONFIG) # Base config
        
        # Use high-precision streaming backtest
        trades, equity_curve = strategy.run_streaming_backtest(
            data_1m,
            vol_mult=vol,
...
def main():
    sym = "ETH_USDT"
    print(f"--- High-Precision Quarter Optimization for {sym} ---")
    
    # Load 1m Data Only (Streaming backtest handles resampling internally)
    ...
    for i in range(4):
        ...
        res = optimize_range(df_1m, start_date, end_date)
        if res:
            res['Period'] = f"Q{i+1}"
            results.append(res)
        else:
